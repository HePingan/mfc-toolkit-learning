...
const routes = ['/', '/dashboard', '/modules/serial', '/labs', '/codegen', '/designer', '/integration', '/reports', '/review', '/planner', '/exam', '/demo-script', '/delivery', '/submit-rehearsal', '/evidence', '/portfolio', '/build-checklist'];
const required = {
  css: ['Android mobile QA v3', 'android-v4-core-loop', 'android-v5-final-handoff', 'android-v6-submit-rehearsal', 'android-v7-evidence-library', 'android-dashboard-continue-card', 'review-list-grid', 'exam-list-card', 'demo-step-list', 'delivery-preview-card', 'delivery-final-five-grid', 'preview-actions', 'submit-step-list', 'submit-status-grid', 'evidence-list-card', 'evidence-status-grid', 'delivery-evidence-library-card'],
  ...
  chunks: {
    ...
    DeliveryPage: ['delivery-quality-v2', 'delivery-gap-wizard', 'final-submit-five', '一键填充模板', '进入真实提交演练', 'android-v7-evidence-library', '证据素材库'],
    SubmitRehearsalPage: ['android-v6-submit-rehearsal', '项目提交演练', 'Submit Order', '导出提交清单', 'next-submit-gap', '证据库关联'],
    EvidencePage: ['android-v7-evidence-library', '证据素材库', 'Evidence Library', 'next-evidence-gap', '导出证据索引'],
  },
};
...
function checkHttpsCertificate() {
  return new Promise((resolve, reject) => {
    ...
    const req = https.get({
      ...
      const wildcard = host.split('.').length > 2 ? `*.${host.split('.').slice(1).join('.')}` : '';
      const matchesHost = names.includes(`DNS:${host}`) || names.includes(host) || (wildcard && names.includes(`DNS:${wildcard}`));
...
async function main() {
  ...
  must(certificate.skipped || certificate.matchesHost, `HTTPS certificate SAN/CN does not include ${new URL(base).hostname}`);
  const results = [];
  ...
  for (const token of required.main) must(mainBundle.text.includes(token), `main JS missing token: ${token}`);

  const assets = { css: cssPath, main: mainPath };
  for (const [chunkName, tokens] of Object.entries(required.chunks)) {
    const chunkAsset = [...home.text.matchAll(new RegExp(`(?:href|src)=\"(\\/assets\\/[^\"]+${chunkName}[^\"]+\\.js)\"`, 'g'))][0]?.[1]
    ...
    for (const token of tokens) must(chunk.text.includes(token), `${chunkName} chunk missing token: ${token}`);
