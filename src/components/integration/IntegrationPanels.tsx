...
export function ProjectImportDiagram() {
  ...
  return (
    <Card className="integration-diagram-card">
      <div className="eyebrow">Project Import Flow</div>
      <h3>网页到 Visual Studio 的集成流</h3>
      <div className="integration-flow">
        {nodes.map((node, index) => (
          <div className="integration-flow-node" key={node}>
  ...
  return (
    <div className="integration-timeline">
      {steps.map((step, index) => (
        <Card className="integration-step-card" key={step.id}>
          <div className="step-index">Step {index + 1}</div>
          <h3>{step.title}</h3>
          <p className="muted">{step.goal}</p>
          <div className="three-col mini-list-grid">
            <div>
              <h4>操作</h4>
              <ul>
...
export function CompileErrorCard({ item }: { item: CompileErrorCase }) {
  return (
    <Card className="compile-error-card">
      <div className="diagram-head compact-head">
        <div>
          <div className="eyebrow">{item.code}</div>
          <h3>{item.title}</h3>
        </div>
        <Link className="badge" to={item.related}>
          相关页面
        </Link>
      </div>
      <p className="warning-text">{item.symptom}</p>
...
export function CompileErrorSearchPanel() {
  ...
  return (
    <Card className="integration-error-search-card">
      ...
      <p className="muted">
        把 Visual Studio 输出窗口里的错误码或关键词贴进来，例如 <code>C2065</code>、
        <code>IDC_</code>、<code>LNK2019</code>、<code>pch.h</code>、<code>UI 卡死</code>。
      </p>
      <div className="search-box integration-error-search-box">
        <input
          aria-label="搜索编译错误"
          placeholder="输入错误码或症状关键词..."
          value={query}
          onChange={(event) => setQuery(event.target.value)}
        />
        <Button className="button-ghost" onClick={() => setQuery('')} type="button">
          清空
        </Button>
      </div>
      <div className="badge-list">
        {quickErrorQueries.map((item) => (
      ...
      <div className="integration-search-results">
        {results.map((item) => (
          <CompileErrorCard item={item} key={`search-${item.code}`} />
        ))}
        {results.length === 0 && (
          <p className="warning-text">
            没有匹配到错误卡。建议先复制错误码，例如 C1083、C2065、LNK2019，或搜索
            pch.h、IDC、Message Map。
          </p>
        )}
      </div>
    </Card>
...
export function ErrorDiagnosisPanel() {
  ...
  const matches = useMemo(() => {
    ...
    return compileErrorCases.filter(
      (item) =>
        lower.includes(item.code.toLowerCase()) ||
        item.causes.some((cause) => lower.includes(cause.slice(0, 8).toLowerCase())) ||
        item.title
          .toLowerCase()
          .split(/[ /]+/)
  ...
  return (
    <Card className="integration-diagnosis-card">
      ...
      <textarea
        className="integration-error-textarea"
        placeholder="粘贴 Visual Studio Error List / Output 里的几行错误信息，例如：error C2065: 'IDC_BTN_SERIAL_OPEN': undeclared identifier"
        value={rawError}
      ...
      <div className="diagnosis-result">
        {matches.length === 0 ? (
          <p className="muted">
            粘贴错误信息后，这里会自动匹配错误卡。建议先保留错误码、文件名、行号和完整英文提示。
          </p>
        ) : (
          <>
            ...
            {matches.map((item) => (
              <div className="diagnosis-match" key={`diag-${item.code}`}>
                ...
                <Link className="inline-link" to={item.related}>
                  打开相关页面
                </Link>
              </div>
            ))}
...
export function IntegrationChecklistPanel() {
  const [checked, setChecked] = useLocalStorage<string[]>(
    storageKeys.localIntegrationChecklist,
    [],
  );
  ...
  const toggle = (item: string) =>
    setChecked(
      checked.includes(item) ? checked.filter((current) => current !== item) : [...checked, item],
    );
  return (
    <Card className="integration-checklist-card">
      <div className="diagram-head compact-head">
        <div>
          <div className="eyebrow">Persistent Final Checklist</div>
          <h3>集成完成检查清单</h3>
        </div>
        <span className="badge badge-success">
          {doneCount}/{integrationChecklist.length} · {percent}%
        </span>
      </div>
      <div className="progress-bar integration-check-progress">
        <span style={{ width: `${percent}%` }} />
      </div>
      <div className="check-grid">
        {integrationChecklist.map((item) => (
          <label className={checked.includes(item) ? 'checked' : ''} key={item}>
            <input checked={checked.includes(item)} onChange={() => toggle(item)} type="checkbox" />{' '}
            <span>{item}</span>
          </label>
        ))}
      </div>
      <div className="form-row">
        <Button className="button-ghost" onClick={() => setChecked([])} type="button">
          重置清单
        </Button>
        <Button
          className="button-ghost"
          onClick={() => setChecked(integrationChecklist)}
          type="button"
        >
          全部标记完成
        </Button>
      ...
      <p className="muted">
        清单已保存到 localStorage，刷新页面不会丢失；适合边操作 Visual Studio 边逐项打勾。
      </p>
    </Card>
  );
}
...
export function IntegrationExportPanel() {
  ...
  return (
    <Card className="integration-export-card">
      <div className="diagram-head compact-head">
        <div>
          <div className="eyebrow">Export Guide</div>
          <h3>导出本地集成说明</h3>
        </div>
        <span className="badge">Markdown</span>
      </div>
      <p className="muted">
        把 6 步集成流程、编译错误速查和最终检查清单导出，方便在 Windows + Visual Studio
        旁边打开对照。
      </p>
      <div className="form-row">
        <Button onClick={download}>下载集成说明</Button>
