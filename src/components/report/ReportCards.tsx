...
export function ReportSummary({ report }: ReportSummaryProps) {
  const stats = [
    ...
    {
      label: '成就徽章',
      value: `${report.achievementUnlocked}/${report.achievementTotal}`,
      note: '自动解锁学习成果',
    },
  ];
  return (
    <section className="dashboard-grid report-summary-grid">
      {stats.map((item) => (
        <Card key={item.label}>
          <span className="big-number">{item.value}</span>
  ...
  return (
    <Card className="report-export-card">
      ...
      <div className="form-row">
        <button
          className="button"
          onClick={() => downloadMarkdown(`mfc-toolkit-learning-report-${Date.now()}.md`, markdown)}
        >
          导出 Markdown
        </button>
        <button
          className="button button-ghost"
          onClick={() => downloadJson(`mfc-toolkit-learning-report-${Date.now()}.json`, report)}
        >
          导出 JSON
        </button>
      </div>
      <details className="report-preview">
        <summary>预览 Markdown 报告</summary>
