...
export const comicPrompts: ComicPrompt[] = [
  {
    ...
    status: 'prompt-ready',
    learningGoal:
      '让学习者理解“串口配置只是通道，协议帧才是业务语义”，并记住 Modbus RTU 的关键字段。',
    storyboard: [
    ...
    prompt:
      '单页中文知识漫画，欧姆社工程漫画风，深色工业控制实验室背景，清晰分镜但不要真实品牌。主题：串口与 Modbus：一帧数据的旅行。画面包含一名年轻工程师和一个拟人化数据帧角色，数据帧从 MFC 调试工具界面出发，经过 COM 参数门禁、ASCII/HEX 双视图、Modbus RTU 字段拆解区，最终到达 PLC/传感器设备。需要在画面中用中文标签标出：COM口、波特率、数据位、校验位、停止位、ASCII、HEX、从站地址、功能码、起始地址、寄存器数量、CRC低字节在前。右下角加“排错三问：参数一致吗？帧边界对吗？CRC 对吗？”。文字尽量短、清晰、可读。科技蓝紫配色，适合学习网站卡片展示。',
  ...
  },
  {
    ...
    status: 'prompt-ready',
    learningGoal:
      '把“资源控件 → 控件 ID → Message Map → OnBnClicked 处理函数 → 成员变量/DDX”的链路视觉化。',
    storyboard: [
  ...
  },
  {
    ...
    status: 'prompt-ready',
    learningGoal:
      '说明 UI 线程只负责交互，通信/解析/保存应进入工作线程，并用锁和退出标志保护共享数据。',
    storyboard: [
  ...
  },
  {
    ...
    prompt:
      '单页中文知识漫画，清晰 ligne-claire 工程插画风，主题：SQLite 与 INI：配置和历史记录各归其位。画面左侧是轻量 INI 配置抽屉，标签包括端口号、波特率、窗口大小、上次路径；右侧是 SQLite 数据仓库，标签包括设备表、收发历史、错误日志、项目记录。中间有一条原则横幅：小配置用 INI，结构化历史用 SQLite。下方分镜展示错误做法“拼接 SQL 字符串”被警告牌拦住，正确做法是 prepare → bind → step → finalize，并检查 AppData/可写目录。深蓝背景，青绿色高亮，文字简短可读。',
