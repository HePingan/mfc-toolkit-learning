...
function buildMembers(ids: string[]): string {
  const ui = ['CTabCtrl m_tabMain;', 'CListBox m_listLog;', 'CStatic m_staticStatus;'];
  if (has(ids, 'serial')) ui.push('CComboBox m_cmbSerialPort;', 'CEdit m_edtSerialSend;');
  if (has(ids, 'tcp-client'))
    ui.push('CEdit m_edtTcpHost;', 'CEdit m_edtTcpPort;', 'CEdit m_edtTcpSend;');
  ...
  const handlers = [
    ...
    ...(has(ids, 'serial')
      ? ['afx_msg void OnBnClickedSerialOpen();', 'afx_msg void OnBnClickedSerialSend();']
      : []),
    ...(has(ids, 'tcp-client')
      ? ['afx_msg void OnBnClickedTcpConnect();', 'afx_msg void OnBnClickedTcpSend();']
      : []),
    ...(has(ids, 'tcp-server') ? ['afx_msg void OnBnClickedTcpServerStart();'] : []),
    ...(has(ids, 'http-client')
      ? ['afx_msg void OnBnClickedHttpGet();', 'afx_msg void OnBnClickedHttpPost();']
      : []),
    ...(has(ids, 'sqlite-store')
      ? [
          'afx_msg void OnBnClickedSqliteOpen();',
          'afx_msg void OnBnClickedSqliteInsert();',
          'afx_msg void OnBnClickedSqliteQuery();',
        ]
      : []),
    ...(has(ids, 'config-store')
      ? ['afx_msg void OnBnClickedConfigLoad();', 'afx_msg void OnBnClickedConfigSave();']
      : []),
    ...(has(ids, 'worker-thread')
      ? ['afx_msg void OnBnClickedTaskStart();', 'afx_msg void OnBnClickedTaskStop();']
      : []),
  ];
...
function buildDdx(ids: string[]): string {
  ...
  if (has(ids, 'serial'))
    lines.push(
      'DDX_Control(pDX, IDC_CMB_SERIAL_PORT, m_cmbSerialPort);',
      'DDX_Control(pDX, IDC_EDT_SERIAL_SEND, m_edtSerialSend);',
    );
  if (has(ids, 'tcp-client'))
    lines.push(
      'DDX_Control(pDX, IDC_EDT_TCP_HOST, m_edtTcpHost);',
      'DDX_Control(pDX, IDC_EDT_TCP_PORT, m_edtTcpPort);',
      'DDX_Control(pDX, IDC_EDT_TCP_SEND, m_edtTcpSend);',
    );
  if (has(ids, 'http-client'))
    lines.push(
      'DDX_Control(pDX, IDC_EDT_HTTP_URL, m_edtHttpUrl);',
      'DDX_Control(pDX, IDC_EDT_HTTP_BODY, m_edtHttpBody);',
    );
  if (has(ids, 'sqlite-store'))
    lines.push(
      'DDX_Control(pDX, IDC_LIST_SQLITE_ROWS, m_lstSqliteRows);',
      'DDX_Control(pDX, IDC_EDT_SQLITE_PAYLOAD, m_edtSqlitePayload);',
    );
...
function logLine(ids: string[], level: 'Info' | 'Error', expr: string) {
  return has(ids, 'logger')
    ? `m_logger.${level}(${expr});`
    : `AppendLog(_T("${level.toUpperCase()}"), ${expr});`;
}
