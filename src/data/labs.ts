...
const modulePitfalls: Record<string, string[]> = {
  serial: [
    '参数只显示不校验，导致真实设备乱码',
    'ASCII/HEX 边界没处理，奇数字节或非法字符仍允许发送',
    '只记录“成功/失败”，没有记录端口、波特率和原始帧',
  ],
  network: [
    '在 UI 线程直接 connect/recv，现场表现为窗口卡死',
    '把一次 recv 当成完整业务包，忽略粘包/半包',
    '错误日志只写“连接失败”，没有区分 DNS、超时、端口占用和状态码',
  ],
  mfc: [
    '控件 ID、DDX 变量和 Message Map 名称不一致',
    '工作线程直接操作控件而不是 PostMessage 回 UI 线程',
    '没有在 OnInitDialog 初始化默认状态和日志入口',
  ],
  'cpp-core': [
    '对象所有权不清晰，窗口关闭后线程继续访问悬空指针',
    '共享队列无锁或锁内执行耗时 IO',
    '容器访问前不检查空队列、下标范围或生命周期',
  ],
  storage: [
    '配置缺失时没有默认值，首次启动直接失败',
    'SQL 字符串直接拼接用户输入',
    '保存后不做回读验证，现场无法确认写入路径和内容',
  ],
};
