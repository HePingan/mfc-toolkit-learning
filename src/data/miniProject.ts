...
function resourceItems(ids: string[]): ResourceItem[] {
  ...
  if (has(ids, 'tcp-client')) items.push(
  ...
  if (has(ids, 'tcp-server')) items.push(
  ...
  if (has(ids, 'http-client')) items.push(
  ...
  if (has(ids, 'sqlite-store')) items.push(
  ...
  if (has(ids, 'config-store')) items.push(
  ...
  if (has(ids, 'worker-thread')) items.push(
...
export function buildResourceHeader(ids: string[]): string {
  const rows = resourceItems(ids).map((item) => `#define ${item.name.padEnd(28)} ${item.value}`).join('\n');
  return `// resource.generated.h\n// Codegen v6 生成的控件 ID 草图：复制到 resource.h 前先确认没有与现有资源 ID 冲突。\n#pragma once\n\n${rows}\n`;
}

function libs(ids: string[]): string[] {
  return Array.from(new Set(getNativeDependencies(ids).flatMap((dep) => dep.libs).filter((lib) => lib.endsWith('.lib'))));
}

export function buildVisualStudioPropertyPages(ids: string[]): string {
  const libLines = libs(ids).map((lib) => `- ${lib}`).join('\n') || '- 当前选择未产生额外 .lib';
...
export function buildBuildOrderChecklist(ids: string[]): string {
  const steps = [
    ...
    ...(has(ids, 'sqlite-store') ? ['接入 SqliteStore，确认 sqlite3.h/lib/dll，能创建 data/mfc_toolkit.db'] : []),
