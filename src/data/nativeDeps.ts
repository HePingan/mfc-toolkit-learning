...
export const nativeDependencies: NativeDependency[] = [
  {
    ...
    notes: ['COM10 及以上端口建议使用 \\\\.\\COM10 格式', 'ReadFile/WriteFile 可能阻塞，建议放入 WorkerThread', '串口参数必须与设备一致：波特率、数据位、校验位、停止位'],
  },
  {
    moduleId: 'tcp-client',
    ...
    notes: ['connect/recv 不要直接放 UI 线程', '注意 WSAGetLastError 错误码', '工程中可使用 #pragma comment(lib, "ws2_32.lib") 或链接器配置'],
  },
  {
    moduleId: 'tcp-server',
    ...
    notes: ['accept 循环必须放入工作线程', '第一版模板只演示单客户端结构', 'Stop 时要关闭监听 socket 让阻塞 accept 退出'],
  },
  {
    ...
    notes: ['HTTPS 请求需要 WINHTTP_FLAG_SECURE', 'POST JSON 需要 Content-Type: application/json', '网络请求建议放入 WorkerThread 并设置超时'],
  },
  {
    ...
    notes: ['需要准备 sqlite3.h、sqlite3.lib、sqlite3.dll 或静态库', '数据库默认放在 exe 旁边 data/mfc_toolkit.db', '多线程写库要串行化，避免 database is locked'],
  },
  {
    ...
    notes: ['配置目录建议放在 exe 旁边的 config 目录', '发布时注意写入权限', '路径处理使用 Shlwapi.lib'],
  },
];
...
export function buildNativeApiNotes(moduleIds: string[]): string {
  ...
  const body = deps.map((dep) => `## ${dep.title}\n\n- Headers：${dep.headers.join(', ')}\n- Libs：${dep.libs.join(', ')}\n- Native API：${dep.api}\n\n注意：\n${dep.notes.map((note) => `- ${note}`).join('\n')}`).join('\n\n');
  const libLines = Array.from(new Set(deps.flatMap((dep) => dep.libs).filter((lib) => lib.endsWith('.lib')))).map((lib) => `#pragma comment(lib, "${lib}")`).join('\n') || '// 当前选择未产生额外 .lib 依赖';
