...
import {
  ...
  buildChecklistStages,
  buildChecklistStorageKey,
  allBuildChecklistItems,
} from '../data/buildChecklist';
    ...
    <div>
      <section className="section-head">
        <div>
          ...
          <h2>MFC 本地构建检查清单</h2>
          <p className="muted">
            把 /codegen ZIP、/designer 控件布局和 /integration
            集成向导串成一个可勾选流程，适合边操作 Visual Studio 边核对。
          </p>
        </div>
        <span className="badge">
          完成 {doneIds.length}/{allItems.length}
        </span>
      </section>
      ...
      <section className="hero build-check-hero">
        ...
        <p>
          这里不编译 MFC，也不访问真实串口、TCP、HTTP 或
          SQLite；它只负责保存本地实践检查进度，并生成 Markdown 交付清单。
        </p>
          ...
          return (
            <Card className="build-stage-card" key={stage.id}>
              <div className="diagram-head compact-head">
                <div>
                  <div className="eyebrow">{stage.id}</div>
                  <h3>{stage.title}</h3>
                </div>
                <span
                  className={stageDone === stage.items.length ? 'badge badge-success' : 'badge'}
                >
                  {stageDone}/{stage.items.length}
                </span>
              </div>
              <p className="muted">{stage.goal}</p>
              <div className="progress-bar build-stage-meter">
              ...
              <div className="form-row">
                <Button
                  ...
                  onClick={() => markStage(stage.items.map((item) => item.id))}
                >
                  本阶段全完成
                </Button>
              </div>
      ...
      <Card className="build-export-card">
        ...
        <p className="muted">
          可复制给老师、同学或作为项目交付附件。导出内容包含每个阶段的已完成/未完成状态。
        </p>
        <div className="form-row">
