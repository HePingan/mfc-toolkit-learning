...
export function DesignerPage() {
  ...
  const selectedControl = useMemo(
    () => preset.controls.find((control) => control.id === selectedId),
    [preset, selectedId],
  );
    ...
    <div>
      ...
      <Card className="designer-preset-card">
        <div className="diagram-head compact-head">
          <div>
            <div className="eyebrow">Layout Presets</div>
            <h3>选择布局模板</h3>
          </div>
          <span className="badge">{mfcDialogPresets.length} 套模板</span>
        </div>
        <div className="designer-preset-grid">
          {mfcDialogPresets.map((item) => (
            <button
              key={item.id}
