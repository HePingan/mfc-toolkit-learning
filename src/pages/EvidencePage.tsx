...
export function EvidencePage() {
  const [state, setState] = useLocalStorage<EvidenceState>(storageKeys.evidenceLibrary, {
  ...
  return (
    <div>
      <section className="hero evidence-hero">
        ...
        <h2>证据素材库</h2>
        <p>
          集中管理截图、日志、源码、Markdown
          和配置文件路径。提交演练、交付包、演示稿和答辩记录都可以引用同一份证据索引。
        </p>
        <div className="form-row">
          ...
          <Button
            ...
            onClick={() => downloadMarkdown('mfc-evidence-index.md', markdown)}
          >
            导出证据索引
          </Button>
        </div>
      ...
      <section className="evidence-summary-grid">
        ...
        <Card>
          ...
          <span>可提交</span>
          <p className="muted">可直接进入提交包</p>
        </Card>
        <Card>
          <strong>{summary.draft}</strong>
          <span>草稿</span>
          <p className="muted">需要补路径或截图</p>
        </Card>
        <Card>
      ...
      <Card className="evidence-next-card">
        <div className="diagram-head compact-head">
          <div>
            <div className="eyebrow">next-evidence-gap</div>
            <h3>下一条最该补：{summary.next?.title ?? '全部完成'}</h3>
          </div>
        ...
        <div className="form-row">
          {summary.next && (
            <Button
              onClick={() => {
                setActiveId(summary.next.id);
                fillTemplate(summary.next.id);
              }}
            >
              补这条证据
            </Button>
      ...
      <Card className="evidence-filter-card">
        <div className="diagram-head compact-head">
          <div>
            <div className="eyebrow">Evidence Type Filter</div>
            <h3>按类型筛选</h3>
          </div>
      ...
      <section className="evidence-layout">
        <Card className="evidence-list-card">
          ...
          {filtered.map((item) => (
            ...
            >
              ...
              <small>
                {statusLabel[item.status]} · {item.type} · {item.module}
              </small>
            </button>
          ))}
        </Card>
        ...
        <Card className="evidence-detail-card">
          <div className="diagram-head compact-head">
            <div>
              <div className="eyebrow">
                {statusLabel[active.status]} · {active.type} · {active.module}
              </div>
              <h3>{active.title}</h3>
            </div>
          ...
          <label className="field-label">
            ...
            <input
              value={active.path}
              onChange={(event) => updateItem(active.id, { path: event.target.value })}
              placeholder="例如：docs/screenshots/vs-build-success.png 或 logs/serial-2026-xx.txt"
            />
          </label>
          <label className="field-label">
            备注
            <textarea
              ...
              rows={7}
              placeholder="记录截图含义、日志时间、源码位置、验收场景。"
            />
          </label>
          <div className="form-row">
            <Button className="button-ghost" onClick={() => fillTemplate(active.id)}>
              生成路径模板
            </Button>
