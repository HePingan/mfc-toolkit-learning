...
function buildDrills(): DrillItem[] {
  const moduleDrills = modules.map(
    (module): DrillItem => ({
      id: `module-${module.id}`,
      level: '基础',
      ...
      expected: [
        module.description,
        `核心概念：${module.concepts.slice(0, 5).join('、')}`,
        `常见错误：${module.commonMistakes.slice(0, 3).join('；')}`,
      ],
      route: `/modules/${module.id}`,
    }),
  );

  const labDrills = labs.slice(0, 10).map(
    (lab): DrillItem => ({
      id: `lab-${lab.id}`,
      ...
      expected: [
        `实验目标：${lab.summary}`,
        `本地目标：${lab.localMfc.goal}`,
        `控件：${lab.localMfc.controls.slice(0, 4).join('、')}`,
        `验收：${lab.localMfc.acceptance.slice(0, 3).join('；')}`,
      ],
      route: '/labs',
    }),
  );

  const practiceDrills = practiceTemplates.map(
    (item): DrillItem => ({
      id: `practice-${item.id}`,
      level: '进阶',
      category: item.stage,
      ...
      expected: [
        `目标：${item.goal}`,
        `文件：${item.files.join('、')}`,
        `步骤：${item.steps.slice(0, 4).join('；')}`,
        `验收：${item.checks.slice(0, 4).join('；')}`,
      ],
      route: '/practice',
    }),
  );

  const rubricDrills = capstoneRubric.map(
    (item): DrillItem => ({
      id: `rubric-${item.item}`,
      level: '项目验收',
      category: 'Capstone',
      ...
      expected: [
        `权重：${item.score} 分`,
        `验收说明：${item.detail}`,
        '证据建议：界面截图、日志片段、构建结果、导出报告或故障复现记录。',
      ],
      route: '/capstone',
    }),
  );
...
export function ExamPage() {
  ...
  const [state, setState] = useLocalStorage<ExamState>(storageKeys.exam, {
    completed: [],
    notes: {},
  });
  ...
  const setNote = (id: string, value: string) =>
    setState({ ...state, notes: { ...state.notes, [id]: value } });
  ...
  const nextQuestion = () => {
    ...
    const currentIndex = Math.max(
      0,
      pool.findIndex((item) => item.id === active?.id),
    );
  ...
  return (
    <div>
      <section className="hero exam-hero">
        ...
        <h2>面试 / 答辩训练场</h2>
        <p>
          把课程模块、浏览器实验、本地 MFC 实战模板和 Capstone
          评分项转成“能不能讲清楚、能不能拿出证据”的训练题。适合面试前、项目验收前集中演练。
        </p>
        <div className="form-row">
          <Link className="button button-primary" to="/capstone">
            查看最终项目
          </Link>
      ...
      <section className="exam-summary-grid">
        <Card>
          ...
          <span>训练题</span>
          <p className="muted">覆盖模块、实验、实战和验收</p>
        </Card>
        <Card>
          <strong>{percent}%</strong>
          <span>完成度</span>
          <p className="muted">答辩训练记录保存在本地</p>
        </Card>
        <Card>
          <strong>{capstoneRubric.length}</strong>
          <span>验收项</span>
          <p className="muted">对应 Capstone 评分维度</p>
        </Card>
        <Card>
          <strong>{practiceTemplates.length}</strong>
          <span>实战模板</span>
          <p className="muted">用于准备项目证据</p>
        </Card>
        <Card>
          <strong>{undoneCount}</strong>
          <span>未完成题</span>
          <p className="muted">可用随机抽题优先训练</p>
        </Card>
      </section>
      ...
      <Card className="exam-control-card">
        <div className="diagram-head compact-head">
          <div>
            <div className="eyebrow">Drill Scope</div>
            <h3>筛选训练题</h3>
          </div>
          <Button
            className="button-ghost"
            onClick={() =>
              downloadMarkdown('mfc-exam-defense-drills.md', exportExamMarkdown(allItems, state))
            }
          >
            导出答辩记录
          </Button>
        ...
        <div className="exam-random-actions">
          ...
          <Button className="button-ghost" onClick={nextQuestion}>
            下一题
          </Button>
          <Button className="button-ghost" onClick={pickUnfinished}>
            只抽未完成
          </Button>
      ...
      <section className={`exam-layout ${focusMode ? 'focus-mode' : ''}`}>
        ...
        {active && (
          <Card className="exam-drill-card">
            <div className="diagram-head compact-head">
              <div>
                <div className="eyebrow">
                  {active.level} · {active.category}
                </div>
                <h3>{active.title}</h3>
              </div>
              <span className="badge">
                {state.completed.includes(active.id) ? '已完成' : '待演练'}
              </span>
            </div>
            ...
            <label className="exam-answer-box">
              我的回答 / 证据记录
              <textarea
                value={state.notes[active.id] ?? ''}
                onChange={(event) => setNote(active.id, event.target.value)}
                placeholder="例如：我会展示 Visual Studio 编译截图、日志窗口、导出的报告，以及对应源码文件…"
                rows={7}
              />
            </label>
            <div className="form-row">
              <Button onClick={() => toggleDone(active.id)}>
                {state.completed.includes(active.id) ? '取消完成' : '标记已演练'}
              </Button>
      ...
      <Card className="exam-boundary-card">
        ...
        <h3>答辩训练不替代真实本地证明</h3>
        <p>
          网页只帮助你整理问答、参考要点和证据清单。真正能用于面试或项目验收的材料，应来自 Windows +
          Visual Studio + MFC 本地项目：编译结果、运行截图、串口/TCP/SQLite
          日志、源码提交和报告导出。
        </p>
      </Card>
    </div>
