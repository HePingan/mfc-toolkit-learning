  ...
  return (
    <Card className="lab-mfc-hint-card">
      <div className="diagram-head compact-head">
        <div><div className="eyebrow">Local MFC Mapping</div><h3>本实验对应的本地 MFC 实现</h3></div>
        <Link className="button button-ghost" to="/practice">打开本地实战页</Link>
      </div>
      <p className="warning-text">浏览器实验只负责理解概念；以下清单用于 Windows + Visual Studio + MFC 项目落地。</p>
      <p>{lab.localMfc.goal}</p>
      <div className="lab-mfc-grid">
        <div><strong>建议文件</strong>{lab.localMfc.files.map((item) => <code key={item}>{item}</code>)}</div>
        <div><strong>控件 ID</strong>{lab.localMfc.controls.map((item) => <code key={item}>{item}</code>)}</div>
        <div><strong>Message Map</strong>{lab.localMfc.messageMap.map((item) => <code key={item}>{item}</code>)}</div>
        <div><strong>验收点</strong>{lab.localMfc.acceptance.map((item) => <span key={item}>✅ {item}</span>)}</div>
      </div>
    </Card>
  );
}
...
export function LabsPage() {
  ...
  const grouped = useMemo(() => modules.map((module) => ({ module, items: labs.filter((lab) => lab.moduleId === module.id) })).filter((group) => group.items.length > 0), []);
  const filteredLabs = moduleFilter === 'all' ? labs : labs.filter((lab) => lab.moduleId === moduleFilter);
    ...
    <div>
      <section className="section-head">
        <div>
          ...
          <h2>交互实验室</h2>
          <p className="muted">所有实验均为浏览器内模拟，用来理解参数、协议、消息映射、内存、线程和配置读写；真实串口/TCP/SQLite 接入请在 Visual Studio 中实践。</p>
        </div>
        <span className="badge">已完成 {done}/{labs.length}</span>
      </section>

      <section className="hero lab-learning-hero">
        <div className="eyebrow">Simulation → Local Practice</div>
        <h2>每个实验都对应一个本地 MFC 落地清单</h2>
        <p>先在浏览器里跑通概念，再查看建议文件、控件 ID、Message Map 和验收点，把实验迁移到 Visual Studio 项目中。</p>
        <div className="form-row">
          {nextLab ? <span className="badge badge-warning">下一项建议：{nextLab.title}</span> : <span className="badge badge-success">全部实验已完成</span>}
      ...
      <div className="lab-module-overview">
        {grouped.map(({ module, items }) => {
          ...
          return (
            <button className={`lab-module-card ${moduleFilter === module.id ? 'active' : ''}`} key={module.id} onClick={() => setModuleFilter(module.id)} type="button">
              <span>{module.icon}</span>
              <strong>{module.title}</strong>
              <em>{finished}/{items.length}</em>
              <i><b style={{ width: `${percent}%` }} /></i>
            </button>
          );
        })}
        <button className={`lab-module-card ${moduleFilter === 'all' ? 'active' : ''}`} onClick={() => setModuleFilter('all')} type="button"><span>🧪</span><strong>全部实验</strong><em>{done}/{labs.length}</em><i><b style={{ width: `${Math.round((done / labs.length) * 100)}%` }} /></i></button>
      </div>

      <div className="lab-status-grid">
        {filteredLabs.map((lab) => <div className={`lab-status ${progress.completedLabs.includes(lab.id) ? 'done' : ''}`} key={lab.id}><strong>{progress.completedLabs.includes(lab.id) ? '✓' : '○'} {lab.title}</strong><span>{lab.level}</span></div>)}
      </div>
      <LabMatrixDiagram />
      <Tabs items={filteredLabs.map((lab) => ({
        id: lab.id,
        label: `${progress.completedLabs.includes(lab.id) ? '✓ ' : ''}${lab.title}`,
        content: <div><div className="lab-intro"><span className="badge">{lab.level}</span><h3>{lab.title}</h3><p>{lab.summary}</p></div>{labViews[lab.id]}<LocalMfcHint lab={lab} /></div>,
      }))} />
    </div>
  );
}
