    ...
    <div>
      <div className="page-title module-hero-card">
        ...
        <div>
          ...
          <p>{module.subtitle}</p>
          <p className="muted">
            预计 {module.estimatedMinutes} 分钟 · 掌握度 {masteryScore(module.id, progress)}% · 实验{' '}
            {relatedLabs.length} 个 · 测验 {quizCount} 题
          </p>
        </div>
      </div>
      <p className="lead">{module.description}</p>
      {module.chapterSummary && (
        <Card className="chapter-summary-card">
          <div className="eyebrow">Chapter Summary</div>
          <h3>本章学习重点</h3>
          <p>{module.chapterSummary}</p>
        </Card>
      )}
      <ModuleConceptDiagram module={module} />
      <div className="two-col">
        <ConceptCard title="学习目标" items={module.objectives} />
        <ConceptCard title="核心概念" items={module.concepts} />
      </div>
      ...
      {relatedLabs.length > 0 && (
        <ConceptCard
          title="本章交互实验"
          items={relatedLabs.map(
            (lab) =>
              `${progress.completedLabs.includes(lab.id) ? '✓ ' : ''}${lab.title}：${lab.summary}`,
          )}
        />
      )}
      ...
      {module.localPractice && (
        <Card className="local-practice-card">
          ...
          <p className="warning-text">
            真实 MFC 编译、串口、Socket、SQLite 操作需要在 Windows + Visual Studio
            本地完成；本网站只提供模拟、步骤和模板。
          </p>
          <div className="two-col">
            <div>
              ...
              <ol className="step-list compact-step-list">
                {module.localPractice.steps.map((step, index) => (
                  <li key={step}>
            ...
            <div>
              ...
              <ul className="acceptance-list">
                {module.localPractice.acceptance.map((item) => (
                  <li key={item}>✅ {item}</li>
                ))}
              </ul>
            </div>
      ...
      <div className="form-row action-row">
        <Button onClick={() => markModule(module.id)}>
          {completed ? '已完成，再次确认' : '标记本模块完成'}
        </Button>
        {relatedLabs.length > 0 && (
        ...
        {next && (
          <Link className="button button-ghost" to={`/modules/${next.id}`}>
            下一章：{next.title}
          </Link>
        )}
      </div>
