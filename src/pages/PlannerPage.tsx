...
function buildPlan(mode: PlanMode, focus: PlannerState['focus'], weakIds: string[]): PlanItem[] {
  ...
  const baseOrder =
    focus === 'weak' && weakIds.length
      ? unique([...weakIds, ...modules.map((m) => m.id)])
      : modules.map((m) => m.id);
  ...
  if (focus === 'local') {
    items.push(
      {
        id: 'local-codegen',
        day: Math.max(2, Math.floor(days * 0.45)),
        title: '生成 MFC Mini Project 包',
        detail:
          '在代码骨架页选择 Serial/TCP/HTTP/SQLite/Logger/WorkerThread，下载 ZIP 后按集成向导接入 Visual Studio。',
        route: '/codegen',
        kind: '本地实战',
      },
      {
        id: 'local-designer',
        day: Math.max(3, Math.floor(days * 0.55)),
        title: '规划 Dialog 控件与 Message Map',
        detail: '用界面设计器确认控件 ID、按钮事件、资源草图，再回到本地 MFC Dialog。',
        route: '/designer',
        kind: '本地实战',
      },
      {
        id: 'local-build',
        day: Math.max(4, Math.floor(days * 0.7)),
        title: '执行 Visual Studio 构建清单',
        detail: '按项目创建、文件导入、属性页、编译链接、运行验收阶段逐项打勾。',
  ...
  items.push(
    {
      id: 'review-loop',
      day: Math.max(2, Math.floor(days * 0.62)),
      title: '错题与术语复习闭环',
      detail: '打开复习训练台，按薄弱模块筛选卡片，先回忆再翻面，必要时标记稍后复习。',
      route: '/review',
      kind: '复习',
    },
    {
      id: 'trouble-loop',
      day: Math.max(3, Math.floor(days * 0.72)),
      title: '现场故障排查训练',
      detail: '至少完成串口、TCP、MFC 消息映射、SQLite/INI 四类排错案例。',
      route: '/troubleshooting',
      kind: '复习',
    },
    {
      id: 'exam-loop',
      day: Math.max(4, Math.floor(days * 0.82)),
      title: '随机答辩训练',
      detail: '进入答辩训练场随机抽题，补充回答和证据记录，优先完成未演练题。',
      route: '/exam',
      kind: '交付',
    },
    {
      id: 'portfolio-loop',
      day: Math.max(5, Math.floor(days * 0.9)),
      title: '整理作品集材料',
      detail: '把学习进度、本地实战、Capstone 证据整理成 README、简历和项目作品集素材。',
      route: '/portfolio',
      kind: '交付',
    },
    {
      ...
      day: days,
      title: '完成项目演示稿与彩排',
      detail: '生成 3/5/10/15 分钟演示稿，补齐证据缺口并完成至少一次彩排。',
      route: '/demo-script',
      kind: '交付',
    },
    {
      ...
      day: days,
      title: '导出学习报告与交付包',
      detail: '汇总进度、掌握度、错题、Capstone 建议，导出 Markdown/JSON 做阶段复盘。',
      route: '/reports',
      kind: '交付',
    },
  );
...
function exportMarkdown(items: PlanItem[], days: number) {
  const byDay = Array.from({ length: days }, (_, index) => index + 1).map((day) => ({
    day,
...
export function PlannerPage() {
  ...
  const [state, setState] = useLocalStorage<PlannerState>(storageKeys.planner, {
    mode: '14days',
    focus: 'balanced',
    completedPlanItems: [],
  });
  const [activeDay, setActiveDay] = useState(1);
  const weakRows = modules
    .map((module) => ({ module, score: masteryScore(module.id, progress) }))
  ...
  const planItems = useMemo(
    () =>
      buildPlan(
        state.mode,
        state.focus,
        weakRows.map((row) => row.module.id),
      ),
    [state.mode, state.focus, weakRows],
  );
  ...
  const setFocus = (focus: PlannerState['focus']) =>
    setState({ ...state, focus, completedPlanItems: [] });
  ...
  return (
    <div>
      <section className="hero planner-hero">
        ...
        <h2>学习计划生成器</h2>
        <p>
          根据当前进度、薄弱模块和目标节奏，把课程、实验、测验、复习、本地 MFC
          实战和报告导出串成一张可执行计划表。
        </p>
      ...
      <section className="planner-summary-grid">
        <Card>
          ...
          <span>全站进度</span>
          <p className="muted">来自模块、实验、测验综合计算</p>
        </Card>
        <Card>
          <strong>{weakRows.length}</strong>
          <span>薄弱模块</span>
          <p className="muted">掌握度低于 70%</p>
        </Card>
        <Card>
          <strong>{days}</strong>
          <span>计划天数</span>
          <p className="muted">可切换 7/14/30 天</p>
        </Card>
        <Card>
          <strong>{planPercent}%</strong>
          <span>计划完成</span>
          <p className="muted">保存在浏览器 localStorage</p>
        </Card>
      </section>
      ...
      <Card className="planner-today-card">
        <div className="diagram-head compact-head">
          <div>
            <div className="eyebrow">Learning loop v4</div>
            <h3>今日任务</h3>
          </div>
          <span className="badge">
            Day {activeDay} · {todayItems.length || '机动'} 项
          </span>
        </div>
        <p className="muted">
          今天建议按“学习/实验 → 复习 →
          答辩/演示证据”的顺序推进，最后把截图、日志、源码路径写进交付材料。
        </p>
      ...
      <Card className="planner-control-card">
        <div className="diagram-head compact-head">
          <div>
            <div className="eyebrow">Plan Options</div>
            <h3>计划参数</h3>
          </div>
          <Button
            className="button-ghost"
            onClick={() => downloadMarkdown('mfc-study-plan.md', exportMarkdown(planItems, days))}
          >
            导出 Markdown
          </Button>
        </div>
        <div className="planner-options">
          ...
          <label>
            ...
            <select
              value={state.focus}
              onChange={(event) => setFocus(event.target.value as PlannerState['focus'])}
            >
      ...
      <section className="planner-days">
        {Array.from({ length: days }, (_, index) => index + 1).map((day) => {
          ...
          const complete =
            dayItems.length > 0 &&
            dayItems.every((item) => state.completedPlanItems.includes(item.id));
          return (
            <button
              className={activeDay === day ? 'active' : complete ? 'done' : ''}
              key={day}
              onClick={() => setActiveDay(day)}
            >
              Day {day}
              <small>{dayItems.length} 项</small>
            </button>
          );
        })}
      </section>
      ...
      <section className="planner-item-grid">
        {visibleItems.length ? (
            ...
            return (
              <Card className={done ? 'planner-item done' : 'planner-item'} key={item.id}>
                ...
                <div className="form-row">
                  ...
                  <Link className="button button-ghost" to={item.route}>
                    打开任务页面
                  </Link>
                </div>
              </Card>
          ...
          <Card>
            <p className="muted">
              今天为机动日：整理笔记、补错题、回看图解中心，或在 Visual Studio 中继续本地调试。
            </p>
          </Card>
        )}
      </section>
      ...
      <Card className="planner-boundary-card">
        ...
        <h3>计划页只做学习编排</h3>
        <p>
          这里不会连接真实串口、不会编译 MFC、不会访问 TCP/SQLite。计划中的本地实战任务需要在
          Windows + Visual Studio + MFC 环境中完成，网页负责安排路径、保存勾选和导出清单。
        </p>
      </Card>
    </div>
  );
