    ...
    <div>
      <section className="section-head">
        <div>
          ...
          <h2>学习报告与项目交付包</h2>
          <p className="muted">
            把学习进度、实验、测验、错题、本地 MFC 实战和 Capstone 自评汇总成可导出的 Markdown/JSON
            报告。
          </p>
        </div>
      ...
      <section className="hero report-hero">
        ...
        <h2>把学习过程沉淀成可交付结果</h2>
        <p>
          报告用于复盘薄弱点、整理 Visual Studio/MFC 本地项目任务，并作为最终 Capstone
          项目的验收依据。
        </p>
      ...
      <ReportSection title="总体完成情况" eyebrow="Overview">
        <div className="report-overview-grid">
          <div>
            <strong>
              {report.completedModules.length}/{modules.length}
            </strong>
            <span>课程模块</span>
          </div>
          <div>
            <strong>
              {report.completedLabs.length}/{labs.length}
            </strong>
            <span>交互实验</span>
          </div>
          <div>
            <strong>
              {Object.keys(report.quizScores).length}/{quizModuleCount}
            </strong>
            <span>模块测验</span>
          </div>
          <div>
            <strong>{report.wrongQuestions.length}</strong>
            <span>错题数量</span>
          </div>
          <div>
            <strong>
              {report.completedPracticeTasks.length}/{practiceTemplates.length}
            </strong>
            <span>本地实战任务</span>
          </div>
          <div>
            <strong>{report.capstoneChecks.length}/19</strong>
            <span>项目验收项</span>
          </div>
        </div>
      </ReportSection>
          ...
          {report.moduleRows.map((row) => (
            <div className="report-row" key={row.id}>
              <div>
                ...
                <p className="muted">
                  实验 {row.labDone}/{row.labTotal} · 测验 {row.quizScore ?? '未完成'} ·{' '}
                  {row.recommendation}
                </p>
              </div>
          ...
          {modules.map((module) => {
            ...
            return (
              <Card key={module.id}>
                ...
                {moduleLabs.map((lab) => (
                  <div
                    className={`lab-status ${report.completedLabs.includes(lab.id) ? 'done' : ''}`}
                    key={lab.id}
                  >
                    <strong>
                      {report.completedLabs.includes(lab.id) ? '✓' : '○'} {lab.title}
                    </strong>
                    <span>{lab.level}</span>
                  </div>
                ))}
      ...
      <ReportSection title="测验成绩与错题复盘" eyebrow="Quiz / Wrong Questions">
        <div className="two-col">
          <Card>
            <h3>测验成绩</h3>
            {Object.keys(report.quizScores).length === 0 ? (
              <p className="muted">暂无测验成绩。</p>
            ) : (
              Object.entries(report.quizScores).map(([id, score]) => (
                <p key={id}>
                  <strong>{modules.find((m) => m.id === id)?.title ?? id}</strong>：{score} 分
                </p>
              ))
            )}
          </Card>
          <Card className={report.wrongQuestions.length ? 'warning-card' : ''}>
            <h3>错题复盘</h3>
            {report.wrongQuestions.length === 0 ? (
              <p className="success-text">暂无错题。</p>
            ) : (
              report.wrongQuestions.slice(0, 8).map((q) => (
      ...
      <ReportSection title="本地 MFC 实战任务" eyebrow="Visual Studio Practice">
        <div className="report-practice-grid">
          {practiceTemplates.map((task) => (
            <Card
              className={
                report.completedPracticeTasks.includes(task.id)
                  ? 'practice-task-card done'
                  : 'practice-task-card'
              }
              key={task.id}
            >
              <div className="eyebrow">{task.stage}</div>
              <h3>
                {report.completedPracticeTasks.includes(task.id) ? '✅' : '⭕'} {task.title}
              </h3>
      ...
      <ReportSection title="Capstone 交付成熟度" eyebrow="Project Delivery">
        <div className="progress-bar report-capstone-bar">
          <span style={{ width: `${report.capstonePercent}%` }} />
        </div>
        <p className="muted">当前项目交付成熟度：{report.capstonePercent}%</p>
        {report.capstoneChecks.length === 0 ? (
          <p className="muted">暂无已勾选验收项。</p>
        ) : (
          <div className="badge-list">
            {report.capstoneChecks.map((item) => (
