...
function quizToCard(question: QuizQuestion): FlashCard {
  ...
  return {
    ...
    source: '错题',
    difficulty:
      question.difficulty === 'hard' ? '易错' : question.difficulty === 'medium' ? '进阶' : '基础',
    route: '/quiz',
  };
}
...
function buildFlashCards(wrongIds: string[]): FlashCard[] {
  ...
  [...wrongCards, ...glossaryCards, ...labCards, ...moduleCards].forEach((card) =>
...
function formatReviewMarkdown(cards: FlashCard[], state: ReviewState) {
  return `# MFC 训练营复习卡片\n\n${cards
    .map((card, index) => {
      const status = state.masteredIds.includes(card.id)
        ? '已掌握'
        : state.postponedIds.includes(card.id)
          ? '稍后复习'
          : '待复习';
...
export function ReviewPage() {
  ...
  const [reviewState, setReviewState] = useLocalStorage<ReviewState>(storageKeys.review, {
    masteredIds: [],
    postponedIds: [],
    reviewedAt: {},
  });
  ...
  const allCards = useMemo(
    () => buildFlashCards(progress.wrongQuestions),
  ...
  const cards = allCards.filter((card) => {
    const moduleOk =
      moduleFilter === 'all'
        ? true
        : moduleFilter === 'weak'
          ? weakModuleIds.includes(card.moduleId)
          : card.moduleId === moduleFilter;
  ...
  const masteredCount = reviewState.masteredIds.filter((id) =>
    allCards.some((card) => card.id === id),
  ).length;
  const postponedCount = reviewState.postponedIds.filter((id) =>
    allCards.some((card) => card.id === id),
  ).length;
  const today = new Date().toISOString().slice(0, 10);
  const todayReviewedCount = Object.values(reviewState.reviewedAt).filter((date) =>
  ...
  const markMastered = () => {
    if (!current) return;
    setReviewState({
      masteredIds: reviewState.masteredIds.includes(current.id)
        ? reviewState.masteredIds
        : [...reviewState.masteredIds, current.id],
  ...
  const postponeCurrent = () => {
    ...
    setReviewState({
      ...reviewState,
      postponedIds: reviewState.postponedIds.includes(current.id)
        ? reviewState.postponedIds
        : [...reviewState.postponedIds, current.id],
      reviewedAt: { ...reviewState.reviewedAt, [current.id]: new Date().toISOString() },
    });
    next();
  };
  ...
  const exportCards = () =>
    downloadMarkdown('mfc-review-flashcards.md', formatReviewMarkdown(cards, reviewState));
  ...
  return (
    <div>
      <section className="hero review-hero">
        ...
        <h2>复习训练台</h2>
        <p>
          把错题、术语、实验复盘和本地实践验收点做成卡片。先回忆答案，再翻面核对，用于考前复习和项目交付前查漏补缺。
        </p>
      ...
      <section className="review-stats-grid">
        <Card>
          ...
          <span>当前卡片</span>
          <p className="muted">随筛选条件动态变化</p>
        </Card>
        <Card>
          <strong>{progress.wrongQuestions.length}</strong>
          <span>错题卡</span>
          <p className="muted">掌握后可从错题本移除</p>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <span>已掌握</span>
          <p className="muted">保存在 localStorage</p>
        </Card>
        <Card>
          <strong>{postponedCount}</strong>
          <span>稍后复习</span>
          <p className="muted">复习状态刷新不丢</p>
        </Card>
        <Card>
          <strong>{todayReviewedCount}</strong>
          <span>今日复习</span>
          <p className="muted">reviewedAt 记录</p>
        </Card>
      </section>
      ...
      <Card className="review-filter-card">
        <div className="diagram-head compact-head">
          ...
          <Button className="button-ghost" onClick={exportCards}>
            导出当前卡片
          </Button>
        </div>
      ...
      {current ? (
          ...
          <div className="diagram-head compact-head">
            <div>
              <div className="eyebrow">
                Card {cards.indexOf(current) + 1} / {cards.length}
              </div>
              <h3>{current.front}</h3>
            </div>
            <div className="badge-list">
              <span className="badge">{current.source}</span>
              <span className="badge">{current.moduleTitle}</span>
              <span className="badge">{current.difficulty}</span>
            </div>
          </div>
          <div className="flashcard-body">
            {!revealed ? (
              <p className="muted">先在脑中或纸上回答，再点击“翻面看答案”。</p>
            ) : (
              <pre>{current.back}</pre>
            )}
          </div>
          <div className="form-row">
            ...
            <Button className="button-ghost" onClick={postponeCurrent}>
              稍后复习
            </Button>
            <Button className="button-ghost" onClick={markMastered}>
              已掌握
            </Button>
            <Link className="button button-ghost" to={current.route}>
              回到来源页面
            </Link>
          </div>
        </Card>
        ...
        <Card>
          <p className="success-text">当前筛选下没有卡片。可以切换到全部模块或全部来源。</p>
        </Card>
      )}
      ...
      <section className="review-list-grid">
        {cards.slice(0, 18).map((card) => (
          <Card
            className={
              reviewState.masteredIds.includes(card.id)
                ? 'review-mini-card mastered'
                : reviewState.postponedIds.includes(card.id)
                  ? 'review-mini-card postponed'
                  : 'review-mini-card'
            }
            key={card.id}
          >
            <div className="badge-list">
              ...
              <span className="badge">{card.moduleTitle}</span>
            </div>
