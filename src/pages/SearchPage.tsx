...
export function SearchPage() {
  ...
  const results = useMemo(
    () => searchLearningItems(query, type, moduleId).slice(0, 80),
    [query, type, moduleId],
  );
  ...
  return (
    <div>
      <section className="hero">
        ...
        <h2>全站学习搜索</h2>
        <p>
          快速定位模块、章节、实验、测验题、概念和资源。适合复习时直接搜索“串口 / Modbus / 消息映射
          / SQLite”等关键词。
        </p>
      ...
      <section className="stat-grid search-stats">
        <div className="stat-card">
          ...
          <span>匹配结果</span>
          <p>{query ? `关键词：${query}` : '未输入时展示推荐索引'}</p>
        </div>
            ...
            return (
              <article className="card search-result-card" key={result.id}>
                <div className="search-result-head">
                  ...
                  <Link className="button button-ghost" to={result.href}>
                    打开
                  </Link>
                </div>
                <h3>{result.title}</h3>
                ...
                <div className="badge-list">
                  {result.keywords.slice(0, 8).map((keyword) => (
                    <span className="badge" key={`${result.id}-${keyword}`}>
                      {keyword}
                    </span>
                  ))}
                </div>
