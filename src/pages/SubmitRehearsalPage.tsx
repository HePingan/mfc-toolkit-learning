...
import {
  exportSubmitMarkdown,
  submitEvidenceTemplate,
  submitReadiness,
  type SubmitEvidenceItem,
  type SubmitEvidenceStatus,
} from '../data/submitRehearsal';
import {
  evidenceForSubmit,
  evidenceTemplate,
  normalizeEvidence,
...
function normalizeItems(items: SubmitEvidenceItem[]) {
  return submitEvidenceTemplate.map((template) => ({
    ...template,
...
export function SubmitRehearsalPage() {
  const [state, setState] = useLocalStorage<SubmitRehearsalState>(storageKeys.submitRehearsal, {
    items: submitEvidenceTemplate,
  });
  ...
  return (
    <div>
      <section className="hero submit-hero">
        ...
        <p>
          按真实验收顺序检查
          README、源码、编译、运行截图、通讯日志、持久化、演示稿和答辩记录。目标是在提交前知道“缺哪一张图、哪一段日志、哪一个文件”。
        </p>
        <div className="form-row">
          ...
          <Button
            ...
            onClick={() => downloadMarkdown('mfc-submit-rehearsal.md', markdown)}
          >
            导出提交清单
          </Button>
        </div>
      ...
      <section className="submit-summary-grid">
        ...
        <Card>
          ...
          <span>已就绪</span>
          <p className="muted">可直接进入最终包</p>
        </Card>
        <Card>
          <strong>{score.draft}</strong>
          <span>草稿</span>
          <p className="muted">有方向但还缺证据</p>
        </Card>
        <Card>
          <strong>{score.missing}</strong>
          <span>缺失</span>
          <p className="muted">优先补齐</p>
        </Card>
      </section>
      ...
      <Card className="submit-next-card">
        <div className="diagram-head compact-head">
          <div>
            <div className="eyebrow">next-submit-gap</div>
            <h3>下一项最该补：{score.next?.title ?? '全部完成'}</h3>
          </div>
        ...
        <p>
          {score.next ? score.next.required : '所有提交项都已就绪，可以导出清单并进入最终提交。'}
        </p>
        <div className="form-row">
          {score.next && (
            <Link className="button button-primary" to={score.next.route}>
              打开关联页面
            </Link>
          )}
          <Button
            className="button-ghost"
            onClick={() => score.next && fillDraft(score.next.id)}
            disabled={!score.next}
          >
            生成证据模板
          </Button>
      ...
      <section className="submit-layout">
        <Card className="submit-step-list">
          ...
          {items.map((item, index) => (
            <button
              key={item.id}
              className={active.id === item.id ? 'active' : item.status}
            ...
            >
              ...
              <small>
                {statusLabel[item.status]} · {item.route}
              </small>
            </button>
          ))}
        </Card>
        ...
        <Card className="submit-detail-card">
          <div className="diagram-head compact-head">
            <div>
              <div className="eyebrow">{statusLabel[active.status]} · Evidence Detail</div>
              <h3>{active.title}</h3>
            </div>
          ...
          <div className="submit-evidence-box">
            <strong>验收要求</strong>
            <p>{active.required}</p>
          </div>
          <div className="submit-evidence-box">
          ...
          <div className="submit-evidence-box submit-linked-evidence">
            <strong>证据库关联</strong>
            {evidenceForSubmit(evidenceItems, active.id).length ? (
              evidenceForSubmit(evidenceItems, active.id).map((evidence) => (
                <p key={evidence.id}>
                  ...
                  {evidence.title}：{evidence.path || '未填写路径'}
                </p>
              ))
            ) : (
              <p>暂无关联证据。</p>
            )}
            <Link className="inline-link" to="/evidence">
              打开证据素材库统一补路径
            </Link>
          </div>
          <div className="submit-status-grid">
            {(['missing', 'draft', 'ready'] as SubmitEvidenceStatus[]).map((status) => (
              <button
                ...
                onClick={() => updateItem(active.id, { status })}
              >
                {statusLabel[status]}
              </button>
            ))}
          ...
          <label className="field-label">
            ...
            <textarea
              value={active.note}
              onChange={(event) => updateItem(active.id, { note: event.target.value })}
              rows={7}
              placeholder="例如：docs/screenshots/vs-build-success.png；src/MfcToolkitDlg.cpp；logs/serial-2026-xx.txt"
            />
          </label>
          <div className="form-row">
            <Button className="button-ghost" onClick={() => fillDraft(active.id)}>
              生成证据模板
            </Button>
