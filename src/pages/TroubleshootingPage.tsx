  ...
  return (
    <div>
      ...
      <Card>
        <div className="search-box trouble-search-box">
          <label>
            ...
            <input
              ...
              placeholder="搜索：乱码、CRC、UI 卡死、Content-Length、控件 ID、路径..."
              aria-label="搜索故障案例"
            />
          </label>
          <label>
            ...
            <select
              value={category}
              onChange={(e) => setCategory(e.target.value as TroubleCategory | 'all')}
              aria-label="选择故障分类"
            >
              <option value="all">全部分类</option>
              {Object.entries(troubleCategoryLabels).map(([key, label]) => (
        ...
        <p className="muted">
          当前显示 {filtered.length} 个案例。建议每次先自己判断，再提交诊断查看修复步骤。
        </p>
      </Card>
      ...
      <div className="trouble-case-list">
        {filtered.length === 0 ? (
          <Card className="warning-card">
            <h3>没有匹配案例</h3>
            <p className="muted">换一个关键词，例如：串口、TCP、HTTP、MFC、线程、INI。</p>
          </Card>
        ) : (
