export function asciiToHex(input: string): string {
  return Array.from(input).map((char) => char.charCodeAt(0).toString(16).toUpperCase().padStart(2, '0')).join(' ');
}
...
export function hexToAscii(input: string): string {
  ...
  return compact.match(/.{1,2}/g)?.map((hex) => String.fromCharCode(parseInt(hex, 16))).join('') ?? '';
}
