...
export function calculateOverall(progress: ProgressState) {
  ...
  const quizPart = quizModules.filter((id) => progress.quizScores[id] !== undefined).length / quizModules.length;
  return Math.round((modulePart * 0.45 + labPart * 0.35 + quizPart * 0.20) * 100);
}
...
export function masteryScore(moduleId: string, progress: ProgressState) {
  ...
  const labDone = moduleLabs.length ? moduleLabs.filter((lab) => progress.completedLabs.includes(lab.id)).length / moduleLabs.length * 100 : 100;
